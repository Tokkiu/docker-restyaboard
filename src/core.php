...
function sendMail($template, $replace_content, $to, $reply_to_mail = '')
{
    ...
    if ($template) {
        ...
        //命令行直接传参数
        // $mymsg="this is test";
        // $newmsg=str_replace(" ","_",$mymsg);
        // echo "newmsg:".$newmsg."end";
        // pclose(popen("php -f ../libs/mymail.php ".$to." ".$subject." ".$newmsg,"r"));

        //写入文件里拿参数
        $filename=time().(string)rand(0,100).$mean;
        //  echo $filename;
          $myfile=fopen("/usr/share/nginx/html/tmp/txts/".$filename,"w+");
          if(!is_writable("/usr/share/nginx/html/tmp/txts/".$filename)){
          echo "cannot write";
          }
          $txt=$to."\n";
          fwrite($myfile,$txt);
        ...
        pclose(popen("php -f /usr/share/nginx/html/server/php/libs/read.php ".$filename,"r"));
